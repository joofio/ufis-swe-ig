Instance: GOFSH-GENERATED-ID-178
InstanceOf: Bundle
Usage: #example
* type = #transaction
* entry[0].resource = Panodil500mgoralsolutionsachet-SE-PLC-MPD

* entry[=].fullUrl = "MedicinalProductDefinition/Panodil500mgoralsolutionsachet-SE-PLC-MPD"
* entry[+].resource = Panodil500mgoralsolutionsachet-SE-PLC-RA

* entry[=].fullUrl = "RegulatedAuthorization/Panodil500mgoralsolutionsachet-SE-PLC-RA"
* entry[+].resource = Panodil500mgoralsolutionsachet-SE-PLC-APD

* entry[=].fullUrl = "AdministrableProductDefinition/Panodil500mgoralsolutionsachet-SE-PLC-APD"
* entry[+].resource = Panodil500mgoralsolutionsachet-SE-PLC-PPD

* entry[=].fullUrl = "PackagedProductDefinition/Panodil500mgoralsolutionsachet-SE-PLC-PPD"
* entry[+].resource = Panodil500mgoralsolutionsachet-SE-PLC-MID

* entry[=].fullUrl = "ManufacturedItemDefinition/Panodil500mgoralsolutionsachet-SE-PLC-MID"
* entry[+].resource = Panodil500mgoralsolutionsachet-SE-PLC-Ingredient

* entry[=].fullUrl = "Ingredient/Panodil500mgoralsolutionsachet-SE-PLC-Ingredient"