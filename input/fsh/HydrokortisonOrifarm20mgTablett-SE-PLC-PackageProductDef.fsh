Instance: HydrokortisonOrifarm20mgTablett-SE-PLC-PPD
InstanceOf: PPLPackagedProductDefinition
Usage: #inline


* containedItemQuantity = 100 http://spor.ema.europa.eu/v1/lists/200000000014#200000002152 "Tablet"
* packageFor = Reference(HydrokortisonOrifarm20mgTablett-SE-PLC-MPD)
* description = "Burk, 100 tabletter"
* marketingStatus.country = $100000000002#100000000535 "Kingdom of Sweden"
* marketingStatus.status = $100000072052#100000072083 "Marketed"
* packaging.type.coding.system = "https://spor.ema.europa.eu/v1/lists/100000073346"
* packaging.type.coding.code = #100000073523
* packaging.type.coding.display = "Jar"
* packaging.quantity = 1
* packaging.material.coding.system = "https://spor.ema.europa.eu/v1/lists/200000003199"
* packaging.material.coding.code = #200000003215
* packaging.material.coding.display = "High Density PolyEthylene"
* packaging.containedItem.item.reference = Reference(HydrokortisonOrifarm20mgTablett-SE-PLC-MID)
* packaging.containedItem.amount.value = 100
* packaging.containedItem.amount.code = #200000002152
* packaging.containedItem.amount.system = "http://spor.ema.europa.eu/v1/lists/100000110633~200000000014"
* packaging.containedItem.amount.unit = "Tablet"