Instance: Nitrolingual04mgdossublingualspray-SE-PLC-PPD01
InstanceOf: PPLPackagedProductDefinition
Usage: #inline


* containedItemQuantity = 1 http://spor.ema.europa.eu/v1/lists/200000000014#200000022801 "Canister"
* packageFor = Reference(Nitrolingual04mgdossublingualspray-SE-PLC-MPD)
* description = "Sprayflaska, ca 200 doser"
* marketingStatus.country = $100000000002#100000000535 "Kingdom of Sweden"
* marketingStatus.status = $100000072052#100000072083 "Marketed"
* packaging.type.coding.system = "https://spor.ema.europa.eu/v1/lists/100000073346"
* packaging.type.coding.code = #100000073498
* packaging.type.coding.display = "Box"
* packaging.quantity = 1
* packaging.material.coding.system = "https://spor.ema.europa.eu/v1/lists/200000003199"
* packaging.material.coding.code = #200000003529
* packaging.material.coding.display = "Cardboard"
* packaging.containedItem.item.reference = Reference(Nitrolingual04mgdossublingualspray-SE-PLC-MID)
* packaging.containedItem.amount.value = 1
* packaging.containedItem.amount.code = #200000022801
* packaging.containedItem.amount.system = "http://spor.ema.europa.eu/v1/lists/100000110633~200000000014"
* packaging.containedItem.amount.unit = "Canister"