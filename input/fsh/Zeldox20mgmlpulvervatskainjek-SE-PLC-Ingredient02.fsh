Instance: Zeldox20mgmlpulvervatskainjek-SE-PLC-ING02
InstanceOf: PPLIngredient
Usage: #example

* status = #active
* for[0] = Reference(Zeldox20mgmlpulvervatskainjek-SE-PLC-MPD)
* for[+] = Reference(Zeldox20mgmlpulvervatskainjek-SE-PLC-MID01)
* for[+] = Reference(Zeldox20mgmlpulvervatskainjek-SE-PLC-APD)
* role = $100000072050#100000072082 "Excipient"
* substance.code.concept = $SubstanceDefinition#100000085580 "BETADEX SULFOBUTYL ETHER SODIUM"